...
public class MethodInjectionPoint<T, X> extends AbstractCallableInjectionPoint<T, X, Method> {
    ...
    private final InvokableMethod invokableMethod;

    protected MethodInjectionPoint(EnhancedAnnotatedMethod<T, X> enhancedMethod, Bean<?> declaringBean, Class<?> declaringComponentClass, boolean observerOrDisposer, boolean disableMethodLookup, InjectionPointFactory factory, BeanManagerImpl manager) {
...
//        if (!Modifier.isStatic(method.getModifiers()) && Modifier.isPublic(method.getModifiers()) && Modifier.isPublic(method.getDeclaringClass().getModifiers())) {
//            this.invokableMethod = new FooInvokableMethod(method);
//        } else {
            if (disableMethodLookup || Modifier.isStatic(method.getModifiers())) {
                this.invokableMethod = new StaticReflectiveInvokableMethod(method);
            } else {
                this.invokableMethod = new ReflectiveInvokableMethod(method);
            }
//        }
    }
    ...
    public T invokeWithSpecialValue(Object declaringInstance, Class<? extends Annotation> annotatedParameter, Object parameter, BeanManagerImpl manager, CreationalContext<?> ctx, Class<? extends RuntimeException> exceptionTypeToThrow) {
        ...
        try {
            return cast(invokableMethod.invoke(declaringInstance, getParameterValues(annotatedParameter, parameter, manager, ctx, invocationContext)));
