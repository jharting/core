...
import org.jboss.weld.introspector.jlr.temp.Fields;
import org.jboss.weld.introspector.jlr.temp.WeldConstructors;
import org.jboss.weld.introspector.jlr.temp.WeldMethods;
...
public class WeldClassImpl<T> extends AbstractWeldAnnotated<T, Class<T>> implements WeldClass<T> {
    ...
    private final LazyValueHolder<Fields<T>> fields;

    private final LazyValueHolder<WeldConstructors<T>> constructors;

    private final LazyValueHolder<WeldMethods<T>> methods;
    ...
    protected WeldClassImpl(Class<T> rawType, Type type, AnnotatedType<T> annotatedType, LazyValueHolder<Set<Type>> typeClosure, LazyValueHolder<Annotations> annotations, LazyValueHolder<Annotations> declaredAnnotations, ClassTransformer classTransformer) {
        ...
        if (annotatedType == null) {
            this.fields = Fields.of(this, classTransformer);
            this.constructors = WeldConstructors.of(this, classTransformer);
            this.methods = WeldMethods.of(this, classTransformer);
        } else {
            this.fields = Fields.of(this, annotatedType, classTransformer);
            this.constructors = WeldConstructors.of(this, annotatedType, classTransformer);
            this.methods = WeldMethods.of(this, annotatedType, classTransformer);
        }
    ...
    public Collection<WeldField<?, ? super T>> getWeldFields() {
        return fields.get().getFields();
    }

    public Collection<WeldField<?, ? super T>> getDeclaredWeldFields() {
        return fields.get().getDeclaredFields();
    }

    public <F> WeldField<F, ? super T> getDeclaredWeldField(String fieldName) {
        for (WeldField<?, ?> field : fields.get().getDeclaredFields()) {
    ...
    public Collection<WeldField<?, ? super T>> getDeclaredWeldFields(Class<? extends Annotation> annotationType) {
        return fields.get().getDeclaredWeldFields(annotationType);
    }

    public WeldConstructor<T> getDeclaredWeldConstructor(ConstructorSignature signature) {
        return constructors.get().getWeldConstructor(signature);
    }
    ...
    public Collection<WeldField<?, ? super T>> getWeldFields(Class<? extends Annotation> annotationType) {
        return fields.get().getWeldFields(annotationType);
    }
    ...
    public Collection<WeldConstructor<T>> getWeldConstructors(Class<? extends Annotation> annotationType) {
        return constructors.get().getWeldConstructors(annotationType);
    }

    public WeldConstructor<T> getNoArgsWeldConstructor() {
        return constructors.get().getNoArgsWeldConstructor();
    }
    ...
    public Collection<WeldMethod<?, ? super T>> getDeclaredWeldMethodsWithAnnotatedParameters(Class<? extends Annotation> annotationType) {
        return methods.get().getDeclaredWeldMethodsWithAnnotatedParameters(annotationType);
    }
    ...
    public Collection<WeldMethod<?, ? super T>> getWeldMethods() {
        return methods.get().getWeldMethods();
    }

    public WeldMethod<?, ?> getDeclaredWeldMethod(Method method) {
        // TODO Should be cached
        for (WeldMethod<?, ?> annotatedMethod : getDeclaredWeldMethods()) {
    ...
    public Collection<WeldMethod<?, ? super T>> getDeclaredWeldMethods() {
        return methods.get().getDeclaredWeldMethods();
    }

    public <M> WeldMethod<M, ?> getDeclaredWeldMethod(MethodSignature signature) {
        for (WeldMethod<?, ? super T> method : getDeclaredWeldMethods()) {
    ...
    public Set<AnnotatedConstructor<T>> getConstructors() {
        return cast(constructors.get().getWeldConstructors());
    }

    public Set<AnnotatedField<? super T>> getFields() {
        return cast(fields.get().getFields());
    }

    public Set<AnnotatedMethod<? super T>> getMethods() {
        return cast(methods.get().getWeldMethods());
    }
